                           ...
                           PTIME_AA ptime)
{
  ...
  if (!(matrix = generate_permutations(n_perms, N)))
    return ERR;

  for (i = 0; i < n_perms; i++)
  {
    if ((start_time = clock()) == ERR)
      return ERR;

    if ((current_ob = method(matrix[i], 0, N - 1)) == ERR)
      return ERR;

    if ((end_time = clock()) == ERR)
      return ERR;
                                ...
                                int incr, int n_perms)
{
  ...
  if (!method || !file || num_min > num_max || incr <= 0 || n_perms <= 0)
    return ERR;
  ...
  {
    /* (i - num_min) / incr is the n-1th iteration of this loop. e.g. when the loop starts, i = num_min, so (i - num_min) / incr = 0 */
